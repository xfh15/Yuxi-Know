...
- 对话页智能体选择器隐藏内置「智能助手」；新建对话优先使用其它主智能体，仅在无其它可选时回退该内置 agent。管理页仍可查看与配置。
- 收紧普通用户权限：侧边栏仅显示“新建对话”和“最近”，工作区、智能体管理、扩展管理及对应后端接口仅管理员可访问，普通用户保留聊天所需的智能体读取、对话历史和运行接口。
- 修复新建对话输入框在输入法候选确认时误将 Enter 识别为发送操作的问题。
- 新增线程级网站资料抓取：Demo Agent 可显式调用 `crawl_website`，通过 Tavily Crawl 收集同主机页面与 PDF，按来源、大小及 SSRF 边界原子发布文件；支持内容指纹短路、部分成功、QA 独立失败，并提示后续问答先检索文件和引用原始 URL；工具运行时参数由 LangGraph 正确注入，工具元数据仅暴露业务参数，避免真实 ToolNode 调用缺参或管理界面无法加载工具；资料发布后直接结束本轮 Agent Run，不再重复读取、搜索和核验生成文件，并由工具侧输出含页面/PDF/Q&A 数量的可读总结（默认日语，在工具区外侧展示）；候选页超过额度时用模型按业务价值排序，新闻/动态类降权；PDF 优先文本抽取并并发解析，避免逐页 OCR 拖慢；抓取期间新增阶段性进度和长耗时心跳，资料概览、QA 及索引默认按用户消息语言生成（日语为默认语言）；工具显示名、标签、配置提示、描述及参数说明改为日语。
- 调整网站资料抓取额度：网页来源与 PDF 来源分开计数（网页最多 30 个、PDF 最多 20 个），PDF 原始文件总量单独限制为 50 MiB；修复模型返回格式化或个别无效引用时 QA 整体失败的问题，自动规范化并过滤不可验证的问题，保留可验证问答并正确记录 QA 状态。
- 优化网站资料问答速度：优先一次读取 `qa.md`，已有答案时直接作答；只有无法回答时才按 `index.md` 精准读取来源，避免每次先执行全目录 `glob`、`grep` 和多次 `read_file`。
- 新增中、英、日三语切换：前端界面、工具调用状态与认证提示支持即时切换，API 请求透传区域信息，后端按请求语言翻译结构化响应；语言偏好会在浏览器本地持久化。自动生成对话标题时根据用户请求语言切换提示词，纯 URL 或无法判断时跟随界面语言。同步修复智能体编辑弹窗因调用缺失的配置名翻译函数而渲染失败，避免后续切换菜单触发 Vue DOM 空引用。
